...
;-----------------------------------------------------------------------------------------------------
    ...
    TVLCT, r, g, b,/Get
    output_dir='E:\OneDrive\IDLworks\浣��\灏����'
   ...
   ; wb=sqrt(wbx^2+wby^2+wbz^2)
    ...
    CONTOUR,ABS(wb)^2,tm[*],f0[*],position=[0.1,0.5,0.9,0.69],xticklen=-0.02,yticklen=-0.02,$
      ystyle=1,xstyle=1,zstyle=1,yrange=[0,40],zrange=[-5,max(ABS(wb)^2)],background=0,color=0,nlevels=Nlevels,/fill,$
      ytitle='B_frequency(mHz)',charsize=1.2,$
