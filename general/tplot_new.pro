...
pro tplot_new,datanames,      $
  ...
  def_opts= {ymargin:[4.,2.],xmargin:[12.,12.],position:fltarr(4), $
    ...
    charsize:chsize,noerase:0,overplot:0,spec:0,scat:0}
  ...
  if keyword_set(polyfill) then begin
    str_element,def_opts,'polyfill.tstart',tstart
    str_element,def_opts,'polyfill.tend',tend
    timebar_shadow,pos,tstart,tend,trg,_extra=def_opts.polyfill
  endif
  ...
  for i=0,nd-1 do begin
    ...
    for d=0,nd2-1 do begin
      ...
      if newlim.scat ne 0 then begin
        routine='scatplot'
      endif else begin
        if newlim.spec ne 0 then routine='specplot' else routine='mplot'
      endelse
  ...
  if keyword_set(vtitle) then begin                 ; finish var_labels
    ...
    xpos = pos[0,nd-1] - 10 * xspace;(def_opts.xmargin[0]-1) * xspace
    ypos = pos[1,nd-1] - 1.5 * yspace
    xyouts,xpos,ypos,vtitle,/norm,charsize=chsize;,font=!p.font
  endif
