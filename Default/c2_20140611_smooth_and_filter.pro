pro c2_20140611_smooth_and_filter
     
      filename='E:\IDLziliao\ly\c2_up_fgm_20140611_v01.cdf'
      loadcdf,filename,'Epoch__C2_UP_FGM',time
      loadcdf,filename,'B_xyz_gse__C2_UP_FGM',B_xyz
      row=n_elements(time)
      Bgsm=dblarr(row,3)
...
;----------------------------------------------------------------------------------------------------
      ...
      for i=0L,row-1L do begin
        ...
        doy=julday(6,11,2014)-julday(1,1,2004)+1
        geopack_recalc,2014,doy,hr,mn,sc
      ...
      index=where(hour gt 7.5 and hour le 8.2)
      Bx_m=Bgsm[index,0]
      By_m=Bgsm[index,1]
      Bz_m=Bgsm[index,2]
      tm=hour[index]
...
;----------------------------------------------------------------------------------------------------      
      ...
      TVLCT, r, g, b,/Get
      output_dir='E:\IDLziliao\ly\'
      device,filename=output_dir+'c2_smooth_and_fliter.ps',xsize=5.5,ysize=6.5,xoffset=1,yoffset=2,/inches,/encapsulated
      !p.multi=[0,1,4,0,0]
      loadct,38
      device,decomposed=0
