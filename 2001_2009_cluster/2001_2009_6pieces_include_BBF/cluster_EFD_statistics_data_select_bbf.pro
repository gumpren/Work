 pro cluster_EFD_statistics_data_select_bbf
    Re=6371.0
...
;      endfor
;      
;     ; t_save=time_v[index_bbf]      ; t_save=tt_bbf
...
;      tinterpol_mxn,'Beta_clip_deflag','velocity_bbf',/NEAREST_NEIGHBOR,/IGNORE_NANS
;      
;      
;      
...
;        velocity_bbf_temp=tsample('velocity_bbf',[tbeg[(index_terval[jj])],tend[(index_terval[jj])]],times=t_c3cis_temp)
...
;          append_Array,velocity_bbf,TEMPORARY(velocity_bbf_temp)
...
;      append_Array,velocity_bbf1,TEMPORARY(velocity_bbf)
...
;  
;    save,save_time_c3fgmcis,t_c3fgm1,t_c3cis1,B_total1,B_gsm1,pos_gsm1,density1,velocity_bbf1,temperature1,pressure1,tt_bbf_save,Beta1,$;$
;      filename=root_dir+'c3_fgmcis_data_selected_'+names[kk]+'_bbf3.sav'
;  
;    undefine,t_c3fgm1,t_c3cis1,B_total1,B_gsm1,pos_gsm1,density1,velocity_bbf1,temperature1,pressure1,beta1;,tt_bbf_save
;  endfor
;  save_time_c3fgmcis=(systime(1)-start)/3600.0
;  print,'save_time ',save_time_c3fgmcis,' hour'
;  
;   stop
 ...
 ; ________________________________part3_______________________
  ...
  filename=file_search(root_dir+'*'+'_bbf3.sav')
  ...
  for i=0,5 do begin
    ...
    indexv=where(abs((velocity_bbf1[*,0])[indext]) le 2000.0)    ; V [-2000,2000]
    indexn=where((density1[indext])[indexv] le 10.0 and (density1[indext])[indexv] gt 0.001 )  ;n [0.001,10]             
...
;    tt_bbf[i]=tt_bbf_save[index_all]
    ...
    velocity_x=velocity_bbf1[index_all,0]
    velocity_y=velocity_bbf1[index_all,1]
    velocity_z=velocity_bbf1[index_all,2]
    ...
    velocity_bbf[i]=[[velocity_x],[velocity_y],[velocity_z]]
    ...
    event_vx[i]=return_vari_event((pos_gsm[i])[*,0],(pos_gsm[i])[*,1],(velocity_bbf[i])[*,0],reverse_gap)
    event_vy[i]=return_vari_event((pos_gsm[i])[*,0],(pos_gsm[i])[*,1],(velocity_bbf[i])[*,1],reverse_gap)

    H_Re[i]=return_thermal_energy_flow_density(pressure[i],(velocity_bbf[i])[*,0],(velocity_bbf[i])[*,1],(velocity_bbf[i])[*,2],2)
    ...
    K_Re[i]=return_kinetic_energy_flow_density(density[i],(velocity_bbf[i])[*,0],(velocity_bbf[i])[*,1],(velocity_bbf[i])[*,2],2)
  ...
  save,t_c3cis,B_gsm,pos_gsm,density,temperature,pressure,velocity_bbf,   $ ;tt_bbf_save, 
    filename=root_dir+'raw_data'+save_str+'_list_bbf_median3.sav'
    
  save,counts,eventimes,event_n,event_t,event_p,event_vx,event_vy,H_Re,K_Re,H_K_Re,     $
    event_hx,event_kx,event_h_k_x,event_hy,event_ky,event_h_k_y,filename=root_dir+'event_data'+save_str+'_list_bbf_median3.sav'
