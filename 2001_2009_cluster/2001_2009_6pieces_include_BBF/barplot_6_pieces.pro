pro barplot_6_pieces
  ...
  restore,filepath('event_data'+save_str+'_list_all_plus_median3.sav',root_dir=root_dir)
  restore,filepath('raw_data'+save_str+'_list_all_plus_median3.sav',root_dir=root_dir)
...
;  h_Re_x=list(a,/ex)
 ...
 ;   h_re_x[i]=(h_re[i])[*,0]
  ...
  title0=[['B!dz!n!x>0 [05,30]min','B!dz!n!x>0 [30,90]min','B!dz!n!x>0 gt 90min'],$
          ['B!dz!n!x<0 [05,30]min','B!dz!n!x<0 [30,90]min','B!dz!n!x<0 gt 90min']]

  ;  ;v      change : title,not change; image, change
  ;yrange change : title, change  ;  image, change
  idx=where(data eq 0)
  data[idx]=!values.f_nan
  pos=set_plot_position(2,3,left=0.01,right=0.84,xgap=0.06,ygap=0.09,low=0.01,high=0.96)
  !p.multi=[0,3,2,0,0]
...
;     
    if j eq 0 then begin
      cgHistoplot, data[*,j,i], /FILL, POLYCOLOR='royal blue', position=pos[i,j,*] ,xtitle='Hx' $
                 , charsize=1.2, yticklen=0.05 , max_value=50, BINSIZE=0.02, maxinput=0.5, MININPUT=-0.2 
    endif else begin
      cgHistoplot, data[*,j,i], /FILL, POLYCOLOR='royal blue', position=pos[i,j,*] ,xtitle='Hx' $
       , charsize=1.2, yticklen=0.05 , max_value=50, BINSIZE=0.02, maxinput=0.5, ytitle=' ', MININPUT=-0.2
