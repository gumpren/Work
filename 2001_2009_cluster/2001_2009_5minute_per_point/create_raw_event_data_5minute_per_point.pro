...
;  ________________________________part3_______________________
    ...
    filename=file_search(root_dir+'c3_fgmcisefw_data_selected'+'*.sav')
    ...
    for jj = 0, 59 do begin
      if (jj ge 0) and (jj le 29)  then t_last_beg[jj]=(jj*5+0)*60.
      if (jj ge 30) and (jj le 59)  then t_last_beg[jj]=((jj-29)*5+0)*60.
    endfor
...
;      index_pos=where(pos_gsm1[index_all,1] gt 0.0  and pos_gsm1[index_all,0] lt -15.0)  ; far_dawnward
;      index_pos=where(pos_gsm1[index_all,1] le 0.0  and pos_gsm1[index_all,0] lt -15.0)  ; far_duskward
;      index_pos=where(pos_gsm1[index_all,1] gt 0.0  and pos_gsm1[index_all,0] ge -15.0)  ; near_dawnward
      index_pos=where(pos_gsm1[index_all,1] le 0.0  and pos_gsm1[index_all,0] ge -15.0)  ; near_duskward
...
;      event_hx,event_kx,event_h_k_x,event_hy,event_ky,event_h_k_y,filename=root_dir+'event_data'+save_str+'_list_5minute_per_point_dawnflank.sav'
   
    save,t_c3cis,density,temperature,pressure,velocity_gsm,B_gsm,pos_gsm,E_gsm_y,H_Re,K_Re,H_K_Re,    $
      filename=root_dir+'raw_data'+save_str+'_list_5minute_per_point_near_duskflank.sav'
    stop
