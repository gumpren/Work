pro plot_line_hx_lasttime_5minute_per_point
  ...
  title_char='line_hx_lasttime_5minute_per_point_0_margin_far_duskflank_normal_average';

  title0=['median_hx_Bz','average_hx_Bz']
  ytitle='H'+cgsymbol('sub')+'x'       
  
  restore,filename=root_dir+'raw_data'+save_str+'_list_5minute_per_point_0_margin_far_duskflank.sav'
  x=indgen(30)
...
;    average_hx1[i]=cal_average((H_Re[i])[*,0],/fix_average)   ;fix average

    median_hx2[i]=median((H_Re[i+30])[*,0])
    average_hx2[i]=cal_average((H_Re[i+30])[*,0],/normal_average)    ; 57 58 
...
;        '[5.5,6.0]','[6.0,6.5]','[6.5,7.0]','[7.0,7.5]','[7.5, 8.0]'],/add
      ...
      cgplot,x,get_Data[*,0,i],position=pos[i,0,*],xrange=[0.5,29.5],yrange=[-0.4,0.4],_extra=opt_plot,/normal,/noerase
