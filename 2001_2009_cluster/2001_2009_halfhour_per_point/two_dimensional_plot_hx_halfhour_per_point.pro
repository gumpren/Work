pro two_dimensional_plot_hx_halfhour_per_point
  ...
  title_char='two_dimensional_plot_hx_Bz_Ge_0_halfhour_per_point';
  
  restore,filename=root_dir+'event_data'+save_str+'_list_halfhour_per_point.sav'

  ;cgdisplay
  cgps_open,output_dir+title_char+save_str+'.ps',xsize=6.0,ysize=7.0
  pos=set_plot_position(4,4,left=0.01,right=0.60,xgap=0.03,ygap=0.03,low=0.01,high=0.96)
  ...
  for jj = 0, 15 do begin
    if (jj ge 0) and (jj le 15)  then duration[jj]=[jj*0.5+0,jj*0.5+0.5]
;    if (jj ge 30) and (jj le 59)  then duration[jj]=[(jj-30)*5+0,(jj-30)*5+5]
  ...
  for i=0,3 do begin
    for j=0,3 do begin
      ...
      str_start=string((duration[j+i*4])[0],format='(f3.1)')
      str_end=string((duration[j+i*4])[1],format='(f3.1)')
      ptitle[i,j]='B!dz!n!x>0 ['+strcompress(str_start,/remove)+','+strcompress(str_end,/remove)+']'
    endfor
  endfor
