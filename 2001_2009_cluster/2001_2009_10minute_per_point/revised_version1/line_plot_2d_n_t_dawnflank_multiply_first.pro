...
pro line_plot_2d_n_t_dawnflank_multiply_first
  ...
  for i=0,29 do begin
    vari_dawn[i]=density[i]*temperature[i]
  endfor
  tt_dawn=t_last
  
  restore,filename=root_dir+namestr+'raw_data'+save_str+'_list_10minute_per_point'+region_strs[2]+'.sav'
  for i=0,29 do begin
    vari_dusk[i]=density[i]*temperature[i]
  endfor  
  tt_dusk=t_last
  
  title_char=namestr+'line_plot_2d_n_t_dawnflank_multiply_first'
...
;             ['S-IMF Dawnflank','S-IMF Duskflank']]
  title_arr=['Dawnflank p*n'];,'Duskflank'
